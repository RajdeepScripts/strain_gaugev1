#!/usr/bin/env python3
"""
single_channel_ind570.py — Single-channel + Total weight interface (IND570 over TCP)
Same design, fonts, and logic as basic_interface_ind570.py, but with only one
channel column and a Total weight panel on the right showing the same value.
"""
...
class SharedState:
    ...
    def push(self, kg, bytes_rx):
        with self._lock:
            ...
            # result_kg (channel display, total panel, the OIML error-test
            # wizard, Delta) sees the same, already-calibrated value.
...
# ── Total weight panel ────────────────────────────────────────────────────────
# Right-hand panel mirroring the single channel's value — same value, shown
# again as a running "Total weight" readout.
...
class TotalWeightPanel(QWidget):
    def __init__(self, parent=None):
        ...
        row = QHBoxLayout()
        self._display = SegmentDisplay(height=80)
    ...
    def update_value(self, display_val: float, unit: str):
        self._unit_label.setText(unit)
        self._display.setText(f"{display_val:.3f}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Single-Channel Interface — SW {SOFTWARE_ID}")
        self.showMaximized()
    ...
    # ── 1 channel column + Total weight panel side by side ──────────────────
    ...
    def _build_channels(self, root):
        ...
        row.addWidget(self._total_panel)
    ...
    def _on_mode_changed(self, mode: str):
        ...
        self._total_panel._unit_label.setText(unit)
    ...
    def _on_delta(self):
        ...
        # calibration/zero — total panel mirrors the same channel.
    ...
    def _update_ui(self):
        ...
        if d['valid'] == 1:
            ...
            for ch in self._channels:
                display_val = ch.update_value(val, unit, kg, mode, self._apply_mode)
            # Total weight mirrors the single channel's already-computed
            # displayed value (same calibration/zero/delta logic applied).
            self._total_panel.update_value(display_val, unit)
